...
export default function ServiceBaruClient({ teknisiList, barangList, userId }: Props) {
  ...
  const removeSparepart = (itemIdx: number, spIdx: number) => {
    setItems((prev) => prev.map((item, i) => (i === itemIdx ? { ...item, spareparts: item.spareparts.filter((_, j) => j !== spIdx) } : item)));
  };
  ...
  const filteredBarang = (idx: number) => {
    ...
    if (!q) return [];
    return barangList.filter((b) => (b.nama.toLowerCase().includes(q) || b.kode.toLowerCase().includes(q)) && !items[idx]?.spareparts.some((s) => s.barang_id === b.id)).slice(0, 6);
  };

  const handleSubmit = async () => {
    if (!noNota.trim()) {
      toast.error("No nota wajib diisi");
      return;
    }
    ...
    if (!tipeHP.trim()) {
      toast.error("Tipe HP wajib diisi");
      return;
    }
    if (items.length === 0) {
      toast.error("Tambah minimal 1 item service");
      return;
    }
    for (const item of items) {
      if (!item.jenis_kerusakan.trim()) {
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              value={noNota}
              onChange={(e) => setNoNota(e.target.value.toUpperCase())}
              placeholder="SVC/2506/001"
              className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 text-gray-500 font-mono uppercase"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={namaCustomer} onChange={(e) => setNamaCustomer(e.target.value)} className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 text-gray-500" />
          </div>
          <div>
          ...
          <div className="col-span-2">
            ...
            <input
              ...
              placeholder="misal: iPhone 14 Pro Max"
              className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 text-gray-500"
            />
          </div>
          <div className="col-span-2">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => addItem("mesin")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-purple-50 text-purple-700 border border-purple-200 hover:bg-purple-100 transition">
              <Wrench className="w-3.5 h-3.5" /> Service Mesin
            </button>
            <button onClick={() => addItem("interface")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition">
        ...
        {items.map((item, idx) => (
          <div key={idx} className={cn("bg-white rounded-xl border p-5 space-y-4", item.tipe === "mesin" ? "border-purple-100" : "border-blue-100")}>
            {/* Item header */}
            <div className="flex items-center justify-between">
              <span className={cn("flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full", item.tipe === "mesin" ? "bg-purple-50 text-purple-700" : "bg-blue-50 text-blue-700")}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input
                  ...
                  onChange={(e) => updateItem(idx, "jenis_kerusakan", e.target.value)}
              ...
              <div>
                ...
                <select
                  value={item.teknisi_id}
                  onChange={(e) => updateItem(idx, "teknisi_id", e.target.value)}
              ...
              <div className="col-span-2">
                ...
                <textarea
                  ...
                  rows={2}
                  placeholder="Detail kondisi, catatan tambahan..."
                  className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 text-gray-500 resize-none"
                />
              </div>
            </div>
                  ...
                  <div key={spIdx} className="flex items-center gap-2 p-2.5 rounded-lg bg-gray-50">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400">
                        Garansi: {sp.garansi_hari ? `${sp.garansi_hari} hari` : "-"} · Stok: {sp.stok}
                      </p>
                    </div>
                    <div className="flex items-center gap-1">
                      ...
                      />
                      <button onClick={() => removeSparepart(idx, spIdx)} className="p-1 rounded hover:bg-red-50 text-gray-400 hover:text-red-500 transition">
                  ...
                  {sparepartSearch[idx] && (
                    <div className="absolute top-full mt-1 left-0 right-0 bg-white rounded-lg border border-gray-100 shadow-lg z-10 overflow-hidden">
                      {filteredBarang(idx).map((b) => (
                        <button key={b.id} type="button" onClick={() => addSparepart(idx, b)} className="w-full text-left px-3 py-2.5 hover:bg-gray-50 border-b border-gray-50 last:border-0 transition">
                      ...
                      {filteredBarang(idx).length === 0 && <p className="text-center text-xs text-gray-400 py-3">Tidak ditemukan atau stok 0</p>}
                    </div>
                  )}
                </div>
              </div>
      ...
      {items.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="flex gap-3">
            <button onClick={() => router.back()} className="flex-1 py-3 rounded-xl border border-gray-300 text-gray-500 text-sm hover:bg-gray-50 transition">
              Batal
            </button>
            <button onClick={handleSubmit} disabled={saving} className="flex-1 py-3 rounded-xl bg-gray-900 text-white text-sm hover:bg-gray-700 transition disabled:opacity-60 flex items-center justify-center gap-2">
